...
import { useAppContext, useJobsContext, initJobState, useMapContext } from '../../contextAPI/context'
import { useFetchJobsHook } from '../../hooks'
import { FormRow, FormRowSelect, FlashMessage, Loading } from '../../components'
import { jobTypeOptions, jobCategoryOptions, statusOptions } from '../../utils/jobOptions'
...
const AddJob = () => {
    ...
    const initFormState = (jobs && isEditMode) ? jobs.find(job => job._id === editJobId) : initJobState
    ...
    const handleSubmit = async (e) => {
        ...
        if(company?.trim() === '') {
            return setFlash({type: 'danger', message: 'Company field can not be blank'})
        }

        if(location?.address?.trim() === '') {
            return setFlash({type: 'danger', message: 'Job address field can not be blank'})
        }
        ...
        try {
            ...
            if((initFormState.location.address !== location.address) && (location.address.trim() !== '')) {
                updateObject = {
                    ...updateObject,
                    location: {
                        ...formValues.location,
